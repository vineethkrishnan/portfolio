...
![A developer at their desk watching a laptop screen show a completed database restore with a green checkmark, a large production-labelled tape reel beside the laptop and smaller test tapes in the background, flat editorial illustration, warm lighting.](/blog/backup-verify-hero.png)

**TL;DR** — I had run dozens of test-backup restore drills and felt safe. What I had never done was pull a real production snapshot, decrypt it, and bring it up in a sandbox. The day I finally did, it worked — even a big database came back clean. `backup-verify` is the tiny Docker Compose lab I ran it through, and this post is half the story and half the tool.
...
I opened `backup-verify` — the throwaway Docker Compose lab I keep around exactly for this. Picked the Postgres profile. Pulled the latest production snapshot out of the storage box, decrypted it, dropped the `.dump` file into `~/Downloads`, and ran `pg_restore` from inside the container.
...
## backup-verify, the tool

`backup-verify` is not a service. There is no CLI, no scheduler, no diff engine. The whole project is a single `docker-compose.yml`, about 175 lines of it, and a `data/` folder it creates as it boots. That is it.
...
Two things in this file sound boring on paper but are the reason I open the repo instead of putting it off.
...
Every time I show someone this repo, the suggestion is the same: "Why don't you wrap it in a CLI that does the restore, counts rows, and reports?"
...
The compose file is small enough that copying it into your own dotfiles takes five minutes. It is at [github.com/vineethkrishnan/backup-verify](https://github.com/vineethkrishnan/backup-verify) if you want the version I actually use.
