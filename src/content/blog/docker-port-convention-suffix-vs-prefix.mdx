...
The committed base file stays clean. No host-side ports at all, just the container-side ones needed for health checks or internal networking.
...
The template file, committed with a sensible default mapping using prefix `3` (the lesson we still keep from the detour above).

```yaml
# docker-compose.override.yml.example (committed)
# Copy this to docker-compose.override.yml on first clone.
# Change any port here if it collides with another project on your machine.
services:
  web:
    ports:
      - "380:80"
