...
> **TL;DR:** A suffix convention for Docker host ports (`5432 -> 54323`) looks clean until the result goes over 65535 and Docker refuses to start. Prefix (`5432 -> 35432`) keeps the math in bounds for almost every common dev port. And if you do not want to bake any of this into the repo, a gitignored `docker-compose.override.yml` is still the simplest answer.
...
Nothing inside the containers changed. Apache still listens on `80`, Postgres still listens on `5432`, and containers on the same Docker network talk to each other on the original ports. Only the laptop-facing port number moved. So no `.env` edits, no Apache config rewrites, nothing downstream to chase.
